...
document.addEventListener('keydown', e => {
  ...
  if (e.shiftKey && !e.metaKey && !e.ctrlKey) {
    ...
    const map = {
      ...
      'T': () => applyTheme(false),
      'D': () => applyTheme(true),
      'Y': () => applyTheme(window.matchMedia('(prefers-color-scheme: dark)').matches),
      'C': () => {
...
cmdOverlay?.querySelectorAll('[data-action]').forEach(el => {
  el.addEventListener('click', e => {
    ...
    if (a === 'copy-link') {
      ...
      closeCmd();
    } else if (a === 'light') { e.preventDefault(); applyTheme(false); closeCmd(); }
    else if (a === 'dark')  { e.preventDefault(); applyTheme(true);  closeCmd(); }
    else if (a === 'sys')   {
      e.preventDefault();
...
fetch('https://api.github.com/users/Aseel012')
  ...
  .then(d => {
    ...
    document.querySelectorAll('.gh-avatar').forEach(el => {
      if (el.tagName === 'IMG') el.src = d.avatar_url;
    });
  })
  .catch(() => {});
...
const PROJ_DATA = {
  timmo: {
    name: 'Timmo — Focused Workspace Timer',
    desc: 'Timmo is a productivity workspace app for coders who want distraction-free session tracking with real-time leaderboards, streak analysis, and session analytics.',
    tags: ['React','Tailwind CSS','Shadcn UI','MongoDB','Node.js'],
    features: [
      'Pomodoro + custom timer with visual session rings.',
      'Global leaderboard with weekly/monthly ranking.',
      'Streak tracking and contribution-style activity grid.',
      'Dark/light glassmorphism UI with smooth Framer Motion transitions.',
    ],
    live: 'https://github.com/aseel012',
    liveLabel: 'View on GitHub ↗',
  },
  retro: {
    name: 'Retro Photo Card Generator',
    desc: 'Generate vintage aesthetic photo cards in seconds — pick frames, upload images, add custom captions, choose retro color palettes and download instantly.',
    tags: ['React','Tailwind CSS','Canvas API','Next.js'],
    features: [
      'HTML5 Canvas engine for real-time card rendering.',
      'Multiple vintage frame styles with custom font overlays.',
      'Colour swatch picker for nostalgic palette control.',
      'One-click high-quality PNG export.',
    ],
    live: 'https://github.com/aseel012',
    liveLabel: 'View on GitHub ↗',
  },
...
document.querySelectorAll('[data-proj]').forEach(card => {
  ...
  card.querySelector('.pcard-trigger')?.addEventListener('click', () => {
    toggleProj(card);
    card.querySelector('.pcard-trigger')?.setAttribute('aria-expanded', String(card.classList.contains('open')));
  });
});
